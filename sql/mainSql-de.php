...
$totalResult = mysqli_query($conn, 'SELECT * FROM `settlementsde` WHERE `total_amount`');
...
$sqlPromotion = "SELECT  Settlement_ID,amount_description,amount_type,amount FROM settlementsde Where amount_type='Promotion' and amount_description='Principal'";
$allPrincipal = mysqli_query($conn, $sqlPromotion);

//total on Promoted cost
$sqlPromotionAmount = "SELECT SUM(COALESCE(amount, 0.00)) AS`amount_sum` FROM `settlementsde` WHERE `amount` and amount_description='Principal' and amount_type='Promotion'";
$allPromotedPrincipal = mysqli_query($conn, $sqlPromotionAmount);

//total on Shipping Cost
$sqlshipping = "SELECT amount_description, SUM(COALESCE(amount, 0.00)) AS`amount_sum` FROM `settlementsde` WHERE `amount` and amount_description='shipping'  ";
$allshippingAmount = mysqli_query($conn, $sqlshipping);

//total Shipping chargeback
$sqlShippingback = "SELECT amount_description, SUM(COALESCE(amount, 0.00)) AS`amount_sum` FROM settlementsde WHERE amount_description='shippingChargeback'";
$allShippingChargeback = mysqli_query($conn, $sqlShippingback);

//List Service fee 
$sqlListServiceFee = "SELECT settlement_id, transaction_type, amount FROM settlementsde WHERE transaction_type = 'ServiceFee'";
$allListServiceFee = mysqli_query($conn, $sqlListServiceFee);

//Total Service fee
$sqlServiceFee = "SELECT transaction_type, SUM(COALESCE(amount, 0.00)) AS`amount_sum` FROM settlementsde WHERE transaction_type = 'ServiceFee'";
$allServiceFee = mysqli_query($conn, $sqlServiceFee);

//Total Refund fee
$sqlTotalRefundfee = "SELECT transaction_type, SUM(COALESCE(amount, 0.00)) AS`amount_sum` FROM settlementsde WHERE transaction_type = 'Refund'";
$allrefundFee = mysqli_query($conn, $sqlTotalRefundfee);

//Total Order Fee
$sqlTotalOrder = " SELECT transaction_type, SUM(COALESCE(amount, 0.00)) AS`amount_sum` FROM settlementsde WHERE transaction_type = 'order'";
...
$sqlTotalamountSettlement ="select settlement_start_date,settlement_end_date, total_amount from settlementsde WHERE `total_amount`";
...
$sqlGroupRefund = "SELECT DISTINCT settlement_id, transaction_type , sum(amount) AS 'Each_group_Amount' FROM settlementsde WHERE transaction_type = 'refund'  Group By settlement_id";
$allgrouprefund = mysqli_query($conn, $sqlGroupRefund);

//Settlement Date list
$sqldatelist = "SELECT * FROM `settlementsde` WHERE `total_amount";
$allStartandenddate = mysqli_query($conn, $sqldatelist);

//Group service fee breakdwown
$sqlgroupservicefee = "SELECT DISTINCT settlement_id, transaction_type , SUM(COALESCE(amount, 0.00)) AS 'Each_group_Amount' FROM settlementsde WHERE transaction_type = 'ServiceFee'  Group By settlement_id";
$allservicefeegroup = mysqli_query($conn, $sqlgroupservicefee);

//group service lighting fee 
$sqlLightningDealFeeGroup = "SELECT DISTINCT settlement_id, transaction_type , SUM(COALESCE(amount, 0.00)) AS 'Each_group_Amount' FROM settlementsde WHERE transaction_type = 'Lightning Deal Fee'  Group By settlement_id";
$allLightningDealFeeGroup = mysqli_query($conn, $sqlLightningDealFeeGroup);

//Group Removal item Complete
$sqlremoveitemgroup = "SELECT DISTINCT settlement_id, amount_description, SUM(COALESCE(amount, 0.00)) AS`Each_group_Amount` FROM settlementsde WHERE amount_description = 'RemovalComplete' Group By settlement_id";
$allremoveitemgroup = mysqli_query($conn, $sqlremoveitemgroup);

//Group Scubscrption Complete
$sqlSubscriptionFeegroup = "SELECT DISTINCT settlement_id, amount_description, SUM(COALESCE(amount, 0.00)) AS`Each_group_Amount` FROM settlementsde WHERE amount_description = 'Subscription Fee' Group By settlement_id";
$allSubscriptionFeegroup = mysqli_query($conn, $sqlSubscriptionFeegroup);

//group settlement
$sqlgroupsettlmentID = "SELECT DISTINCT settlement_id FROM settlementsde Group By settlement_id";
$allGroupsettlementgroupID = mysqli_query($conn, $sqlgroupsettlmentID);

//froup storage fee
$sqlgroupstoragefee = "SELECT DISTINCT settlement_id, amount_description, SUM(COALESCE(amount, 0.00)) AS`Each_group_Amount` FROM settlementsde WHERE amount_description = 'Storage fee' Group By settlement_id";
$allgroupstoragefee = mysqli_query($conn, $sqlgroupstoragefee);

//Group Reversal Reimbursement
$sqlgroupreversalreimbursement = "SELECT DISTINCT settlement_id, amount_description, SUM(amount) AS`Each_group_Amount` FROM settlementsde WHERE amount_description = 'REVERSAL_REIMBURSEMENT' Group By settlement_id";
$allgroupreversalreimbursement = mysqli_query($conn, $sqlgroupreversalreimbursement);

//Warehouse damage
$sqlgroupwarehousedamage = "SELECT DISTINCT settlement_id, amount_description, SUM(amount) AS`Each_group_Amount` FROM settlementsde WHERE amount_description = 'WAREHOUSE_DAMAGE' Group By settlement_id";
$allgroupwarehousedamage = mysqli_query($conn, $sqlgroupwarehousedamage);

//grouptotalamount
$sqltotalamountgroup = "SELECT  settlement_id, settlement_start_date,settlement_end_date, total_amount FROM settlementsde Group By settlement_id";
$allgrouptotalamount = mysqli_query($conn, $sqltotalamountgroup);

//grouptotalmatch
$sqlgrouptotalmatch = "Select total_amount, Sum(amount) AS total_match_amount_sum from settlementsde Group By settlement_id";
$grouptotalmatch = mysqli_query($conn, $sqlgrouptotalmatch);

//groupbalancematch
$sqlgroupbalancematch = "Select total_amount, Sum(amount) AS match_amount_sum, Sum(total_amount - amount) AS total_match from settlementsde Group By settlement_id";
