<!-- Portfolio Modal Chart -->

<!-- Dashboard UK-->
<div class="portfolio-modal modal fade" id="portfolioModalDashboarduk" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="close-modal" data-dismiss="modal">
                <div class="lr">
                    <div class="rl"></div>
                </div>
            </div>
            <div class="container">
                <div class="row">
                    <div>
                        <div class="modal-body">
                            <h2>Dashboard UK Settlement</h2>
                            <hr class="star-primary">
                            <p>
                                <a href="<?php echo 'Dashboard-uk.php'; ?>" class="btn btn-lg btn-default">UK Dashboard</a><br>
                            </p>

                            <button class="btn btn-success" type="button" data-dismiss="modal">
                                <i class="fa fa-times"></i>
                                Close</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- End Dashboard UK-->
<!-- Dashboard Germany-->
<div class="portfolio-modal modal fade" id="portfolioModaldashboardde" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="close-modal" data-dismiss="modal">
                <div class="lr">
                    <div class="rl"></div>
                </div>
            </div>
            <div class="container">
                <div class="row">
                    <div>
                        <div class="modal-body">
                            <h2>Dashboard Germany Settlement</h2>
                            <hr class="star-primary">
                            <p>
                                <a href="<?php echo 'Dashboard-de.php'; ?>" class="btn btn-lg btn-default">Germany Dashboard</a><br>
                            </p>

                            <button class="btn btn-success" type="button" data-dismiss="modal">
                                <i class="fa fa-times"></i>
                                Close</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<!--End Dashboard Germany-->
<!-- Dashboard Italy-->
<div class="portfolio-modal modal fade" id="portfolioModaldashboardItaly" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="close-modal" data-dismiss="modal">
                <div class="lr">
                    <div class="rl"></div>
                </div>
            </div>
            <div class="container">
                <div class="row">
                    <div>
                        <div class="modal-body">
                            <h2>Dashboard Italy Settlement</h2>
                            <hr class="star-primary">
                            <p>
                                <a href="<?php echo 'Dashboard-it.php'; ?>" class="btn btn-lg btn-default">Italy Dashboard</a><br>
                            </p>

                            <button class="btn btn-success" type="button" data-dismiss="modal">
                                <i class="fa fa-times"></i>
                                Close</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- End Dashboard Italy-->
<!-- Dashboard Spain-->
<div class="portfolio-modal modal fade" id="portfolioModaldashboardSpain" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="close-modal" data-dismiss="modal">
                <div class="lr">
                    <div class="rl"></div>
                </div>
            </div>
            <div class="container">
                <div class="row">
                    <div>
                        <div class="modal-body">
                            <h2>Dashboard Spain Settlement</h2>
                            <hr class="star-primary">
                            <p>
                                <a href="<?php echo 'Dashboard-es.php'; ?>" class="btn btn-lg btn-default">Spain Dashboard</a><br>
                            </p>

                            <button class="btn btn-success" type="button" data-dismiss="modal">
                                <i class="fa fa-times"></i>
                                Close</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- End Dashboard Spain-->
<!-- Dashboard France-->
<div class="portfolio-modal modal fade" id="portfolioModaldashboardFrance" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="close-modal" data-dismiss="modal">
                <div class="lr">
                    <div class="rl"></div>
                </div>
            </div>
            <div class="container">
                <div class="row">
                    <div>
                        <div class="modal-body">
                            <h2>Dashboard France Settlement</h2>
                            <hr class="star-primary">
                            <p>
                                <a href="<?php echo 'Dashboard-fr.php'; ?>" class="btn btn-lg btn-default">France Dashboard</a><br>
                            </p>

                            <button class="btn btn-success" type="button" data-dismiss="modal">
                                <i class="fa fa-times"></i>
                                Close</button>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<!-- End Dashboard France-->
