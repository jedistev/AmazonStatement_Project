    ...
    <body id="page-top">
        ...
        <div class="container">
                ...
                <thead>
                    <tr  Class="table-header-total">
                        ...
                        while ($row = mysqli_fetch_array($allStartandenddate)) {
                            ?>
                            <td><?php echo $row["settlement_start_date"]; ?> - <?php echo $row["settlement_end_date"]; ?></td>
                            <?php
                        };
                        ?>
                    </tr>
            ...
            <?php
                        ...
                        echo '<th>amount_type</th>';
                        echo '<th>amount_description</th>';
                        echo '<th>amount</th>';
                        echo '</tr>';
                        
                        while (($row1 = mysqli_fetch_array($Groupresult, MYSQLI_ASSOC)) != NULL) {
                            echo '<tr>';
                            echo '<td>' . $row1['settlement_id'] . '</td>';
                            echo '<td>' . $row1['amount_description'] . '</td>';
                            echo '<td>' . $row1['Each_group_Amount'] . '</td>';
                            echo '</tr>';
                        }
            ...
            echo '<table class="table table-condensed table-bordered table-striped table-hover dt-responsove wrap" cellspacing="0">';
            echo '<tbody>';
            $resultEachGroup = mysqli_query($conn,$sqlBreakdownAmountGroup);
            while (($row1 = mysqli_fetch_array($resultEachGroup, MYSQLI_ASSOC)) != NULL) {
                echo '<tr>';
                echo '<th class="table-header-total">' . $row1['amount_description'] . '</th>';
                echo '<td class="table-smaller-text">' . $row1['Each_group_Amount'] . '</td>';
                echo '</tr>';
            }
            mysqli_free_result($resultEachGroup);
            echo '</tbody>';
            echo '</table>'
            ?>
            ...
            while (($row1 = mysqli_fetch_array($resultTranactiontypegroup, MYSQLI_ASSOC)) != NULL) {
                ...
                echo '<tr>';
                echo '<th class="table-header-total">' . $row1['transaction_type'] . '</th>';
                echo '<td class="table-smaller-text">' . $row1['settlement_id'] . '</td>';
                echo '<td class="table-smaller-text">' . $row1['Each_group_Amount'] . '</td>';
                echo '</tr>';
            }
