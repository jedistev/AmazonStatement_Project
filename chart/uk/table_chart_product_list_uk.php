<?php
// Database Connection
$TableChartAmountProductList = "Select 
...
HAVING Product_code IS NOT NULL AND LENGTH(Product_code) > 0
Order by Product_code ASC";
$ResulttableChartAmountProductList = mysqli_query($conn, $TableChartAmountProductList);
?> 
...
<?php
while ($row = mysqli_fetch_array($ResulttableChartAmountProductList)) {
    echo "['" . $row['Product Code'] . "'," . $row['Order Quantity'] . "," . $row['Reimbursement Quantity'] . "," . $row['Refund Quantity'] . "],";
}
?>
        ]);


        var table = new google.visualization.Table(document.getElementById('table_chart_Product_list_div'));
